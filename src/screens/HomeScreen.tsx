...
export default function HomeScreen() {
  ...
  const [analytics, setAnalytics] = useState<FollowerGrowth[]>([]);
  ...
  const fetchFollower = async () => {
    const response = await getFollowerAnalytic();
    // console.log("response :", response);
    // console.log("followerGrowth:", analytics);
    setAnalytics(response);
  };
  useEffect(() => {
    const loadData = async () => {
      try {
        await Promise.all([fetchUser(), fetchCount(), fetchFollower()]);
      } catch (error) {
        console.log(error);
      } finally {
        setLoading(false);
